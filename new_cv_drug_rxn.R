...
#R crash at this step, not enough memory, maybe need to be done directly in PostgreSQL to link to HLT term in meddra
# meddra<-tbl(hcopen,'meddra')%>%dplyr::select(PT_Term,HLT_Term,HLGT_Term)%>%as.data.table()
...
cv_drug_rxn_new_2006 <- cv_drug_rxn_new %>%
...
BCPNN_result <- BCPNN_HCSC(input_df, MC = TRUE, NB.MC = 10000)
...
GPS_result1 <- GPS(input_df, RANKSTAT = 2)
GPS_quantile <- as.data.frame(GPS_result1$ALLSIGNALS) %>% arrange(drug, event)
GPS_result2 <- GPS(input_df, RANKSTAT = 3)
...
cv_reportid <- cv_drug_rxn_new_2006 %>%
  dplyr::select(PT_NAME_ENG, quarter) %>%
...
mastertable_pt_new<-BCPNN_result[,1:7] %>%
  inner_join(PT_count[,c(1,2,4,5)], by = c("drug_code", "event_effect")) %>%
